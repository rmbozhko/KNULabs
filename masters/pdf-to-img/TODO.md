...
- Config: Set your id2label and label2id mapping (e.g., {0: "CONTENT", 1: "INSTRUCTION"}).
...
## Testing:
1. The model will predict labels for every token. Write a script that iterates through the predictions and groups tokens with the same label if their bounding boxes are adjacent (the "Proximity Threshold" logic). -> derive block-level predictions from token-level predictions.
...
1. PDF -> OCR -> Model Inference -> Label Aggregation -> Excalidraw JSON